  ...
  return (
    <div className="cal-nav">
      <button
        ...
        onClick={() => actions.nudgeCalOffset(-1)}
      >
        ‹
      </button>
      <div className="cal-nav-label">{label}</div>
      <button className="cal-nav-btn" onClick={() => actions.nudgeCalOffset(1)}>
        ›
      </button>
      <button
        className="cal-today-btn"
        onClick={() => {
          actions.setCalOffset(0);
          actions.setSelectedMonthDay(null);
        }}
      >
        today
      </button>
            ...
            <div className="day-hour-slot">
              {hTasks.map((t) => {
                ...
                return (
                  <div
                    key={t.id}
                    className={`day-block day-block--timed${t.done ? " day-block--done" : ""}`}
                    onClick={() => actions.openDetail(t.id)}
                  >
                    <div className="day-block-time">
                      {fmtRange(t.startTime, t.endTime)}
                      {dur ? ` · ${fmtDur(dur)}` : ""}
                    </div>
                    <div
        ...
        return (
          <div className="week-day-block" key={dk}>
            <div className="wdb-header">
              <span className="wdb-name">
                {DAY_NAMES[d.getDay()].toUpperCase()}
              </span>
              <span className={`wdb-num${isToday ? " wdb-num--today" : ""}`}>
                {d.getDate()}
              </span>
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  actions.openSheet({ prefillDate: dk });
                }}
              >
                +
              </button>
              ...
              {dayTasks.length === 0 ? (
                ...
                dayTasks.map((t) => {
                  ...
                  return (
                    <div
                      key={t.id}
                      className={`cal-chip${t.startTime ? " cal-chip--timed" : ""}${t.done ? " cal-chip--done" : ""}`}
                      onClick={() => actions.openDetail(t.id)}
                    >
                      ...
                      <span
                        className={`cc-title${t.done ? " cc-title--done" : ""}`}
                      >
                        {t.title}
                      </span>
                      {t.subtasks.length > 0 && (
    ...
    <div className="month-day-panel month-day-panel--open">
      ...
      {dayTasks.map((t) => (
        ...
        >
          ...
          <span className={`mdp-title${t.done ? " mdp-title--done" : ""}`}>
            {t.title}
          </span>
          {t.startTime && (
            <span className="mdp-time">{fmtRange(t.startTime, t.endTime)}</span>
          )}
        </div>
      ))}
    </div>
  ...
  return (
    <>
      <div className="month-view">
        <div className="month-grid">
          {["S", "M", "T", "W", "T", "F", "S"].map((n, i) => (
            <div className="month-wday" key={i}>
              {n}
            </div>
          ))}
            ...
            return (
              <div
                ...
                className={[
                  ...
                  isOther ? "month-day--other" : "",
                  isSelected ? "month-day--selected" : "",
                ]
                  .filter(Boolean)
                  .join(" ")}
                onClick={() =>
                  actions.setSelectedMonthDay(isSelected ? null : dk)
                }
              >
                ...
                <div className="month-day-dots">
                  {dayTasks.slice(0, 3).map((t) => (
                    <div
                      ...
                      className={[
                        "month-dot",
                        t.startTime ? "month-dot--timed" : "",
                        t.done ? "month-dot--done" : "",
                      ]
                        .filter(Boolean)
                        .join(" ")}
                    >
                  ...
                  {dayTasks.length > 3 && (
                    <div className="month-more">+{dayTasks.length - 3}</div>
                  )}
                </div>
              </div>
            );
      ...
      {state.selectedMonthDay && (
        <MonthDayPanel dayKey={state.selectedMonthDay} />
      )}
    </>
  );
}
